...
namespace XLua
{
    public partial class ObjectTranslator
    {
        ...
        class IniterAdderUnityEngineVector2
        {
			...
			static void Init(LuaEnv luaenv, ObjectTranslator translator)
			{
				...
				translator.RegisterPushAndGetAndUpdate<Assets.Scripts.Framework.Manager.UILayer>(translator.PushAssetsScriptsFrameworkManagerUILayer, translator.Get, translator.UpdateAssetsScriptsFrameworkManagerUILayer);
        ...
        int AssetsScriptsFrameworkManagerUILayer_TypeID = -1;
		int AssetsScriptsFrameworkManagerUILayer_EnumRef = -1;
        
        public void PushAssetsScriptsFrameworkManagerUILayer(RealStatePtr L, Assets.Scripts.Framework.Manager.UILayer val)
        {
            if (AssetsScriptsFrameworkManagerUILayer_TypeID == -1)
            {
			    bool is_first;
                AssetsScriptsFrameworkManagerUILayer_TypeID = getTypeId(L, typeof(Assets.Scripts.Framework.Manager.UILayer), out is_first);
				
				if (AssetsScriptsFrameworkManagerUILayer_EnumRef == -1)
				{
				    Utils.LoadCSTable(L, typeof(Assets.Scripts.Framework.Manager.UILayer));
				    AssetsScriptsFrameworkManagerUILayer_EnumRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);
				}
				
            }
			
			if (LuaAPI.xlua_tryget_cachedud(L, (int)val, AssetsScriptsFrameworkManagerUILayer_EnumRef) == 1)
            {
			    return;
			}
			
            IntPtr buff = LuaAPI.xlua_pushstruct(L, 4, AssetsScriptsFrameworkManagerUILayer_TypeID);
            if (!CopyByValue.Pack(buff, 0, (int)val))
            {
                throw new Exception("pack fail fail for Assets.Scripts.Framework.Manager.UILayer ,value="+val);
            }
			
			LuaAPI.lua_getref(L, AssetsScriptsFrameworkManagerUILayer_EnumRef);
			LuaAPI.lua_pushvalue(L, -2);
			LuaAPI.xlua_rawseti(L, -2, (int)val);
			LuaAPI.lua_pop(L, 1);
        ...
        public void Get(RealStatePtr L, int index, out Assets.Scripts.Framework.Manager.UILayer val)
        {
		    LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA )
            {
			    if (LuaAPI.xlua_gettypeid(L, index) != AssetsScriptsFrameworkManagerUILayer_TypeID)
				{
				    throw new Exception("invalid userdata for Assets.Scripts.Framework.Manager.UILayer");
				}
                ...
                if (!CopyByValue.UnPack(buff, 0, out e))
                {
                    throw new Exception("unpack fail for Assets.Scripts.Framework.Manager.UILayer");
                }
				val = (Assets.Scripts.Framework.Manager.UILayer)e;
                
            }
            else
            {
                val = (Assets.Scripts.Framework.Manager.UILayer)objectCasters.GetCaster(typeof(Assets.Scripts.Framework.Manager.UILayer))(L, index, null);
            }
        }
		
        public void UpdateAssetsScriptsFrameworkManagerUILayer(RealStatePtr L, int index, Assets.Scripts.Framework.Manager.UILayer val)
        {
            ...
            if (LuaAPI.lua_type(L, index) == LuaTypes.LUA_TUSERDATA)
            {
                ...
                if (!CopyByValue.Pack(buff, 0,  (int)val))
                {
                    throw new Exception("pack fail for Assets.Scripts.Framework.Manager.UILayer ,value="+val);
                }
            }
	...
	public partial class StaticLuaCallbacks
    {
	    internal static bool __tryArrayGet(Type type, RealStatePtr L, ObjectTranslator translator, object obj, int index)
		{
			...
			else if (type == typeof(Assets.Scripts.Framework.Manager.UILayer[]))
			{
			    Assets.Scripts.Framework.Manager.UILayer[] array = obj as Assets.Scripts.Framework.Manager.UILayer[];
				translator.PushAssetsScriptsFrameworkManagerUILayer(L, array[index]);
				return true;
			}
