...
namespace XLua.CSObjectWrap
{
    ...
    public class AssetsScriptsFrameworkManagerUILayerWrap
    {
		public static void __Register(RealStatePtr L)
        {
		    ObjectTranslator translator = ObjectTranslatorPool.Instance.Find(L);
		    Utils.BeginObjectRegister(typeof(Assets.Scripts.Framework.Manager.UILayer), L, translator, 0, 0, 0, 0);
			Utils.EndObjectRegister(typeof(Assets.Scripts.Framework.Manager.UILayer), L, translator, null, null, null, null, null);
			
			Utils.BeginClassRegister(typeof(Assets.Scripts.Framework.Manager.UILayer), L, null, 5, 0, 0);

            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "Back", Assets.Scripts.Framework.Manager.UILayer.Back);
            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "Mid", Assets.Scripts.Framework.Manager.UILayer.Mid);
            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "Tip", Assets.Scripts.Framework.Manager.UILayer.Tip);
            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "Loading", Assets.Scripts.Framework.Manager.UILayer.Loading);
            

			Utils.RegisterFunc(L, Utils.CLS_IDX, "__CastFrom", __CastFrom);
            
            Utils.EndClassRegister(typeof(Assets.Scripts.Framework.Manager.UILayer), L, translator);
        }
        ...
        static int __CastFrom(RealStatePtr L)
		{
            ...
            if (lua_type == LuaTypes.LUA_TNUMBER)
            {
                translator.PushAssetsScriptsFrameworkManagerUILayer(L, (Assets.Scripts.Framework.Manager.UILayer)LuaAPI.xlua_tointeger(L, 1));
            }
			
            else if(lua_type == LuaTypes.LUA_TSTRING)
            {

			    if (LuaAPI.xlua_is_eq_str(L, 1, "Back"))
                {
                    translator.PushAssetsScriptsFrameworkManagerUILayer(L, Assets.Scripts.Framework.Manager.UILayer.Back);
                }
				else if (LuaAPI.xlua_is_eq_str(L, 1, "Mid"))
                {
                    translator.PushAssetsScriptsFrameworkManagerUILayer(L, Assets.Scripts.Framework.Manager.UILayer.Mid);
                }
				else if (LuaAPI.xlua_is_eq_str(L, 1, "Tip"))
                {
                    translator.PushAssetsScriptsFrameworkManagerUILayer(L, Assets.Scripts.Framework.Manager.UILayer.Tip);
                }
				else if (LuaAPI.xlua_is_eq_str(L, 1, "Loading"))
                {
                    translator.PushAssetsScriptsFrameworkManagerUILayer(L, Assets.Scripts.Framework.Manager.UILayer.Loading);
                }
				else
                {
                    return LuaAPI.luaL_error(L, "invalid string for Assets.Scripts.Framework.Manager.UILayer!");
                }
            ...
            {
                return LuaAPI.luaL_error(L, "invalid lua type for Assets.Scripts.Framework.Manager.UILayer! Expect number or string, got + " + lua_type);
            }
